...
export default function Layout({ children, activeRoute, inConsoleMode = false }: LayoutProps) {
  const { isDark, toggleTheme, loggedInUser, logout, socialLinks } = useApp();
  ...
  return (
    ...
    >
      ...
      <SeoManager activeRoute={activeRoute} inConsoleMode={inConsoleMode} />

      {/* NEW INTEGRATIVE NAVIGATION BAR WITH SCROLL EMBEDDING */}
      <Navbar 
        isDark={isDark} 
        onToggleTheme={toggleTheme} 
        onLaunchConsole={() => { window.location.href = '/dashboard'; }}
        inConsoleMode={inConsoleMode}
        onExitConsole={() => { window.location.href = '/'; }}
